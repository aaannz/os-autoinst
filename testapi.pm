...
sub init {
    ...
    my $addr = sockaddr_un($OpenQA::testapi::server::socketname);
    socket($testapi_server, PF_UNIX, SOCK_STREAM, 0);
    connect($testapi_server, $addr);
    $testapi_fd = fileno($testapi_server);
}
...
=cut
sub save_screenshot {
    myjsonrpc::send_json($testapi_fd, { cmd => 'save_screenshot' });
    return;
}
...
sub record_soft_failure {
    myjsonrpc::send_json($testapi_fd, { cmd => 'record_soft_failure', @_ });
    return;
}
...
=cut
sub assert_screen {
    my $token = myjsonrpc::send_json($testapi_fd, {cmd => 'assert_screen', @_});
...
=cut
sub check_screen {
    my $token = myjsonrpc::send_json($testapi_fd, {cmd => 'check_screen', @_});
    my $rsp = myjsonrpc::read_json($testapi_server, $token);
    return $rsp->{result};
}
...
=cut
sub match_has_tag {
    my $token = myjsonrpc::send_json($testapi_fd, {cmd => 'match_has_tag', @_});
    my $rsp = myjsonrpc::read_json($testapi_server, $token);
    return $rsp->{result};
}
...
sub assert_and_click {
    my $token = myjsonrpc::send_json($testapi_fd, {cmd => 'assert_and_click', @_});
    my $rsp = myjsonrpc::read_json($testapi_server, $token);
    return $rsp->{result};
}
...
=cut
sub send_key {
    myjsonrpc::send_json($testapi_fd, { cmd => 'send_key', @_ });
    return;
}
# 
# =head2 hold_key
